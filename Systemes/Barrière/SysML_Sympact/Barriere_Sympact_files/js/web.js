...
function buildTree(li) {
	...
	emptyUL.onExpand = function() {
		...
		XMLRequest.send(resourcesLocation + 'xml/tree/' +node.getAttribute('refid') + '.xml', function(responseXML) {
			if (responseXML) {
		...
		if (childNodes[c].tagName == 'ownedElement') {
				...
				if (members[p].nodeType == 1) {
					...
					if (members[p].getAttribute('isRelationship') == 'true') {
						...
						if (parentUL.firstChild && parentUL.firstChild.elementName == 'Relations')
							...
							var relationLI = addNode(parentUL, 'Relations', 'javascript:void(0);', 'Barrière Sympact_files/icon_929601658.jpg', 'true');
						...
						childNode = addNode(relationUL, name, "javascript: gotoElement('" + members[p].getAttribute('refid') + "');", icon, members[p].getAttribute('hasChild'));
						...
						if (groupBy) {
							...
							childNode = addNode(parentUL, name, "javascript: gotoElement('" + members[p].getAttribute('refid') + "');", icon, members[p].getAttribute('hasChild'));
							childNode.data = members[p];
							childNode.setAttribute('refid', members[p].getAttribute('refid'));
							if (groupMap) {
			...
			for (var m = 0; m < members.length; m++) {
				if (members[m].nodeType == 1) {
					...
					var childNode = addNode(parentUL, name, "javascript: gotoElement('" + members[m].getAttribute('refid') + "');", icon, members[m].getAttribute('hasChild'));
					childNode.data = members[m];
					childNode.setAttribute('refid', members[m].getAttribute('refid'));
					// comment below and uncomment
					// buildTree(childNode) to see diagram inner
					// node
...
// function findNode(nodeId, select, callback) {
 ...
 * 
 * @param value
 *           a HTML element containing value
 * @param element
 *           DOM element
 */
...
function renderBrowser(responseXML) {
	...
	if(responseXML)
	{
		...
		var node = addNode(root, dataModel.getAttribute('name'), "javascript: showSpec('" + dataModel.getAttribute('refid') + "');", dataModel.getAttribute('icon'), dataModel.getAttribute('hasChild'));
		node.data = dataModel;
	...
	/*
	 * var magicdraw; if (responseXML) magicdraw = responseXML.getElementsByTagName('magicdraw')[0]; if (magicdraw !=
	 * null) { showLoading(); var root = document.createElement('ul'); root.id = 'tree'; tree = new Tree(root.id);
	 * tree.image.plus = resourcesLocation + 'images/tree/plus.gif'; tree.image.minus = resourcesLocation +
	 * 'images/tree/minus.gif'; tree.root = root; var dataModel = firstChild(magicdraw); var node = addNode(root,
	 * dataModel.getAttribute('name'), "javascript: showSpec('" + dataModel.getAttribute('refid') + "');",
	 * dataModel.getAttribute('icon')); node.data = dataModel; node.setAttribute('refid',
	 * dataModel.getAttribute('refid')); buildTree(node); var browser = document.getElementById('browser');
	 * browser.appendChild(root); tree.expand(node); hideLoading(); if (gotoElementId != '') { showSpec(gotoElementId,
	 * false); } }
	 */
}
...
function renderModel(responseXML) {
	...
	if (!stopRender) {
		...
		if (usehyperlink) {
			usehyperlink = false;
			/* UPDM : need to check for attributes (may slow) */
			var mdRule = false;
			for (var b = 0; b < model.childNodes.length; b++) {
				if (model.childNodes[b].tagName == 'ownedBehavior') {
					var behaviorTags = model.getElementsByTagName('ownedBehavior');
					if (behaviorTags.length > 0) {
						gotoElement(behaviorTags[0].firstChild.getAttribute('refid'));
					} else {
						renderElement(model);
					...
					if (classType != 'Package' || classType != 'Model' || classType != 'Profile') {
						var diagramTags = model.getElementsByTagName('ownedDiagram');
						if (diagramTags.length > 0) {
							content.model = model;
							XMLRequest.send(resourcesLocation + 'xml/' + diagramTags[0].firstChild.getAttribute('refid') + '.xml', function(responseXML) {
								var magicdraw;
								if (responseXML)
									magicdraw = responseXML.getElementsByTagName('magicdraw')[0];
								if (magicdraw == null) {
			...
			if (!mdRule) {
				...
				} else if (model.tagName == 'collaboration') {
					content.model = model;
				...
				} else if (model.tagName == 'activity' || model.tagName == 'statemachine' || model.tagName == 'interaction' || model.tagName == 'protocolstatemachine' || model.tagName == 'opaquebehavior' || model.tagName == 'functionalbehavior') {
							...
							if (usecontextmenu) {
								XMLRequest.send(resourcesLocation + 'xml/' + elementId + '.xml', function(responseXML) {
									...
									if (model) {
										...
										// callbehavior
										else if (model.tagName == 'callbehavior') {
											var behaviorTags = model.getElementsByTagName('behavior');
											if (behaviorTags.length > 0) {
...
function forward() {
	...
	if (forwardPageId) {
		gotoElement(forwardPageId, true);
