...
\begin{itemize}
 ...
 \item Double-cliquer sur le fichier correspondant au TP et à la version de Matlab utilisée, il doit avoir une extension en \og slx \fg.\\
  ...
  \end{center}
 \item Afin d'exporter des données, il est nécessaire d'insérer un bloc \textbf{To File} disponible dans la section \textif{Sinks} et de le connecter à la donnée à extraire,
 \item Double-cliquer dessus afin de modifier le paramètre \textif{Save format} en \textbf{Array}. Cela a pour effet de créer un fichier \textit{fichier.mat},
 \item Celui-ci peut être convertit en fichier \textit{fichier.csv} en utilisant les commandes suivantes:
\verb?FileData = load('fichier.mat');

\verb?csvwrite('fichier.csv', FileData.ans);
 \end{itemize}	
\end{itemize}
...
Séquence \sequence \ - \type \num \ - Îlot \numprint{\ilot} &  \multirow{3}{*}{\hspace{1cm}\includegraphics[height=1.5cm]{../../img/logo}} &  \begin{flushright} \multirow{4}{*}{\hspace{1cm}\includegraphics[height=4cm]{"\dossierilot/img/qrcode"}}\end{flushright}\\
 \textbf{\institute} \\
 \auteurun\\
 \auteurdeux
