...
\begin{itemize}
 ...
 \item Double-cliquer sur le fichier correspondant au TP et à la version de Matlab utilisée, il doit avoir une extension en \og slx \fg.\\
  ...
  \end{center}
 \item Afin d'exporter des données, il est nécessaire d'insérer un bloc \textbf{To File} disponible dans la section \textif{Sinks} et de le connecter à la donnée à extaire,
 \item Cela a por effet de créer un fichier \textit{.mat}, qui peut être convertit en fichier \textit{.csv} en utilisant les commandes suivantes:
 \begin{itemize}
  \item FileData = load('nom_du_fichier.mat');
  \item csvwrite('nom_du_fichier.csv', FileData.ans);
 \end{itemize}
\end{itemize}
}
