...
\paragraph{Question 3:} Donner l'expression de toutes les caractéristiques de la FTBF, vous pourrez proposer une simplification de l'écriture si vous le jugez nécessaire.

\paragraph{Question 4:} Déterminer le temps de réponse à 5\% et le dépassement D\% de la réponse à un échelon $\alpha_{volant}=\dfrac{\pi}{4}$.
...
\paragraph{Question 5:} L'évolution de la phase obtenue ci-dessus est-elle identique à celle du diagramme de Bode obtenue à la question 6 ? Argumenter votre réponse.
...
$FTBF(p)=\dfrac{K}{1+A.p+B.p^2+C.p^3}$, avec
...
\begin{itemize}
 \item $K_a=\dfrac{34}{57}$,
 \item $K_b=0,04.10^{-3}.\frac{180}{\pi}.m.rad^{-1}$,
 \item $K_c=\dfrac{2.\pi.10^3}{500}rad.m^{-1}$,
 \item $K_d=K_a$,
 \item $k=0,24.10^3m^3.s^{-1}.m^{-1}$,
 \item $S=0,003m^2$,
 \item $M=500kg$,
 \item $f=10^{-10}.dm^3.s^{-1}.Pa^{-1}$,
 \item $\dfrac{V.\beta}{2}=10^{-12}.m^3.Pa^{-1}$
\end{itemize}
...
\begin{itemize}
 ...
 \item $B=0,000004$
 \item $C=0,00000002$
 \item $\omega_0=497,37rad.s^{-1}$,
 \item $\xi=0,18$.
\end{itemize}

 Il est donc possible de négliger la partie devant le $p^3$ afin de se ramener à un second ordre que nous sommes capables d'étudier.
