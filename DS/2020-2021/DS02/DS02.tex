...
\question{Exprimer la fonction de transfert boucle fermée du système $H_1(p)$, sous forme canonique, en fonction de $K$, $K_p$, $K_i$, $\tau$ et $\tau_i$. Préciser l’ordre et la classe de ce système.}
...
Dans les modèles avec prise en compte des frottements la vitesse de rotation du moteur en régime permanent est bien inférieure à celle obtenue sans tenir compte du frottement. Dans le cas d’un effort constant la vitesse atteinte en régime permanent est d’environ $465\ rad.s^{-1}$ et est constante.
...
\reponse{3}{}{
$\frac{\Omega_m(p)}{U_m(p)}=\frac{k_c}{\left(k_e\cdot k_c+R\cdot f_v \right)}\cdot \frac{1}{1+\frac{RJ}{k_e\cdot k_c+R\cdot f_v}}$
}
...
\reponse{5}{}{
$E(p)=K_{capt}=A(p)$\\
$D(p)=\frac{C_m(p)}{U_m(p)}=\frac{k_c}{r}$\\
$M(p)=\frac{1}{J\cdot p+f_v+/frac{k_e \cdot k_c}{R}}$\\
$B(p)= C_{corr}(p)$
}
...
$\begin{cases}
  ...
  H_r(p)=\frac{C_r(p)M(p)}{1+C(p)M(p)} 
\end{cases}$
}
...
$\begin{cases}
	...
	H_1(p)=\frac{K_p.K}{1+K_p.K}\cdot \frac{1}{1+\frac{\tau}{1+K_p.K}}
\end{cases}$
Ordre 1, classe 0
}
...
$\begin{cases}
	...
	avec\;K’=\frac{K_p.K}{1+K_p.K}\;et\;\tau’=\frac{\tau}{1+K_p.K}
\end{cases}$
}
...
\reponse{6}{}{
$H_1(p)=\frac{C(p) \cdot M(p)}{1+C(p) \cdot M(p)}$\\
$H_1(p)=\frac{K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}{1+K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}$\\
$H_1(p)=\frac{K_pK_iK}{(1+\tau_i p)(1+\tau p)+K_pK_iK}$\\
$H_1(p)=\frac{K_pK_iK}{1+K_pK_iK} \cdot \frac{1}{1+\frac{\tau_i+\tau}{1+K_pK_iK}\cdot p+\frac{\tau_i \tau}{1+K_pK_iK} \cdot p^2 }$
ordre 2 classe 0}
...
\reponse{4}{}{
$\varepsilon_{s2}=\lim_{p \to 0}p\cdot \varepsilon(p)=\lim\limits_{p \to 0}p\cdot\frac{C_r(p)M(p)}{1+C(p)M(p)}=\lim\limits_{p \to 0}p\cdot\frac{\frac{C_{r0}}{p}\cdot \frac{K}{1+\tau\;p}}{1+K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}$ \\
$\varepsilon_{s2}=\lim\limits_{p \to 0}\frac{C_{r0}\cdot \frac{K}{1+\tau\;p}}{1+K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}=\frac{K.C_{r0}}{1+K_p\;K_i\;K}$}
...
\reponse{4}{}{
$\varepsilon_{t2}=\lim_{p \to 0}p\cdot \varepsilon(p)=\lim\limits_{p \to 0}p\cdot\frac{C_r(p)M(p)}{1+C(p)M(p)}=\lim\limits_{p \to 0}p\cdot\frac{\frac{C_{r0}}{p^2}\cdot \frac{K}{1+\tau\;p}}{1+K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}$ \\
$\varepsilon_{t2}=\lim\limits_{p \to 0}\frac{1}{p}\;\frac{C_{r0}\cdot \frac{K}{1+\tau\;p}}{1+K_p\frac{K_i}{1+\tau_i\;p} \cdot \frac{K}{1+\tau\;p}}=0$}
...
\reponse{6}{}{
Pour $\omega>100rad.s^{-1}$, on peut l'assymiler à un premier ordre de fonction de transfert $H_r(p)=\frac{2.2}{1+0.004\; p}$
...
Donc $\xi=\sqrt{0.03}\approx0.17$

Et $\omega_0=200rad.s^{-1}$.
}
